...
async def cache_status(
    ...
    """返回缓存系统整体状态。"""
    # WHY: The frontend database/bargain pages have no site-account login flow;
    # this read-only aggregate endpoint must stay public or the page goes blank.
...
async def get_global_bargains_count(
    ...
    """全局捡漏总数。"""
    # WHY: Bargain plaza count is read-only public data; requiring AppUser makes
    # the Vue page fall back to an empty list because both global endpoints 401.
    displayable_items = await _fetch_displayable_global_bargains(db, brand=brand, xd_card=xd_card)
