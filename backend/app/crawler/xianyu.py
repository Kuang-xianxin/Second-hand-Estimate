"""
闲鱼商品数据爬虫 - 仅供个人学习与研究使用

免责声明（重要）：
1. 本模块仅供个人学习、研究和参考使用，禁止用于任何商业目的。
2. 数据来源于公开的闲鱼平台（goofish.com），仅代表当前页面展示的公开信息，
   不代表对平台数据真实性、完整性的任何担保。
3. 使用者应自行承担所有风险，因使用本模块导致的任何直接或间接损失，
   包括但不限于账号封禁、数据丢失、交易损失等，开发者不承担任何责任。
4. 使用者应遵守闲鱼平台的服务条款及当地法律法规。
5. 本模块不对爬取数据的准确性、完整性、时效性作任何保证。
6. 请合理控制爬取频率，避免对目标网站造成负担。
