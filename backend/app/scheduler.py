...
from app.services.cache_updater import (
    backfill_missing_cache_from_crawled_items,
    compute_price_for_keyword,
    batch_update_cache,
    warm_l1_cache,
    write_crawled_items,
)
    ...
    try:
        ...
        async def _progress_callback(**kw):
            await _write_progress(
                ...
                max_items_per_kw=kw.get("max_items_per_kw") or max_items_per_kw or 0,
            )
        ...
        async with db_session_factory() as session:
            ...
            if source_mode == "sweep":
                # WHY: external sweep crawls one representative keyword at a time,
                # so successful runs should also fill missing cache rows from
                # already persisted crawled_items for the same tier.
                backfilled, backfill_failed = await backfill_missing_cache_from_crawled_items(
                    get_keywords_by_tier(tier),
                    session,
                )
                logger.info(
                    "[%s] sweep 缓存回填完成：成功 %s，失败/无样本 %s",
                    tier.value,
                    backfilled,
                    backfill_failed,
                )
