...
async def classify_camera_items_by_llm(keyword: str, items: List[dict]) -> List[dict]:
    ...
    prompt = f"""你是二手相机市场数据清洗助手。请识别并剔除配件商品和型号不符的商品。

目标关键词：{keyword}

【强制排除规则】（满足任意一条立即排除）：
- 商品型号与目标关键词「{keyword}」明显不同。例如目标是「ixus700」，则「a700」「r700」「700d」「ixy700」「ixus 800」等均须排除；目标是「iphone15」则「iphone14」须排除
- 商品是单独的电池、充电器、数据线、屏幕/液晶屏、镜头盖、USB盖、外壳、背带、贴膜、读卡器、内存卡、滤镜等配件
- 商品是维修零件、拆机件、说明书
- 价格低于 200 元且标题不含"整机"/"相机"/"机身"等整机词

【保留规则】：
- 标题或描述中型号与「{keyword}」吻合（允许别名：如 ixus=ixy=digital ixus）
- 价格在 400 元以上的同型号二手相机整机
- 有疑问时，型号明确吻合优先保留
