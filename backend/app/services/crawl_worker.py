...
from app.services.xianyu_failure import normalize_xianyu_debug_summary
...
async def crawl_canary(storage_state_override: Optional[str] = None) -> Tuple[bool, str, dict]:
    ...
    async def _check_one(kw: str) -> Tuple[bool, str, dict]:
        for attempt in range(2):
            if attempt > 0:
                await asyncio.sleep(5)  # 等 5 秒再重试，避免连续请求触发风控
            search_kwargs = {
            ...
            try:
                ...
                login_hint, risk_hint = normalize_xianyu_debug_summary(debug)
...
async def crawl_single_keyword(
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            if storage_state_override:
                search_kwargs["storage_state_override"] = storage_state_override
            if sample_progress_callback:
                loop = asyncio.get_running_loop()

                def _forward_sample_progress(payload: dict):
                    # WHY: crawler.search() runs inside a worker thread; this
                    # bridges its raw item updates back onto the async Redis
                    # progress callback without blocking Playwright.
                    try:
                        asyncio.run_coroutine_threadsafe(
                            sample_progress_callback(keyword, payload),
                            loop,
                        )
                    except RuntimeError:
                        pass

                search_kwargs["progress_callback"] = _forward_sample_progress
            debug = {}
            try:
                if sem:
                    async with sem:
            ...
            # 检查爬虫调试摘要中的登录/风控状态。
            login_required, risk_detected = normalize_xianyu_debug_summary(debug)
            if (login_required or risk_detected) and not items:
                error = "触发风控验证" if risk_detected else "登录态失效"
                return CrawlResult(
                    keyword=keyword,
                    items=[],
                    success=False,
...
async def crawl_all_ccd_models(
    ...
    for batch_start in range(0, total_keywords, batch_size):
        ...
        async def _sample_progress(keyword: str, payload: dict):
            if not progress_callback:
                return
            raw_count = int(payload.get("raw_item_count") or 0)
            sample_count = min(max(raw_count, 0), max_items_per_kw)
            event = payload.get("event") or "crawling"
            await progress_callback(
                stage="crawling",
                done=batch_start + batch_processed,
                total=total_keywords,
                current_keyword=f"{keyword} · 样本 {sample_count}/{max_items_per_kw} · {event}",
                success=success_count,
                fail=fail_count,
                # WHY: for a one-keyword sweep, completed all_items is still
                # empty while Playwright is collecting. Add the current raw
                # count so the UI percent moves during the long crawl stage.
                items=len(all_items) + sample_count,
        ...
        tasks = [
            asyncio.create_task(crawl_single_keyword(
                ...
                sample_progress_callback=_sample_progress,
            ))
            for kw in batch
        ]
        ...
        for task in asyncio.as_completed(tasks):
            ...
            if settings.crawl_stop_on_risk and (result.login_required or result.risk_detected):
                aborted = True
                abort_reason = "触发风控验证" if result.risk_detected else "登录态失效"
